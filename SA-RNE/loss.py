...
class LCPGuidedLoss(nn.Module):
    ...
    def __init__(self, lcp_module, lambda_mse, lambda_tv, lambda_grad, epsilon=1e-3):
    ...
    def _gaussian_target(self, x, kernel_size=9, sigma=2.0):
        """Build a Gaussian-smoothed self-supervised target."""
    ...
    def forward(self, denoised, original):
        ...
        gaussian_target = self._gaussian_target(original).detach()
        loss_noise = self.lambda_mse * F.mse_loss(denoised, gaussian_target)
        ...
        total_loss = loss_noise + loss_grad + loss_tv + loss_lcp
        return total_loss, loss_noise, loss_grad, loss_tv
